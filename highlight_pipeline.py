...
def run_highlight_pipeline(
    ...
    if not transcripts:
        result["message"] = f"ไม่พบ transcript สำหรับวิดีโอ {video_id} — อาจไม่มีคำบรรยาย"
        logger.error(f"❌ {result['message']}")
        return result
    ...
    # ──────────────────────────────────────────
    # Step 3: Analyze with OpenClaw (with retry)
    # ──────────────────────────────────────────
        ...
        for i, highlight in enumerate(highlights):
            ...
            try:
                ...
                if clip_result.get("status") == "success":
                    highlight["clip_url"] = clip_result.get("file_url", "")
                    highlight["clip_status"] = "success"
                    report(f"✅ {clip_label} สำเร็จ", highlight["clip_url"])
                else:
                    highlight["clip_url"] = ""
